...
class LinearModel:
    ...
    def activation(self, x):
        ...
        """
        if type(x) == bool or type(x) == list or type(x) == np.ndarray:
    ...
    def forward(self, inputs):
        ...
        """
        return self.activation(inputs[0]*self.weights[0] + inputs[1]*self.weights[1] + self.weights[2])
    ...
    def loss(prediction, label):
        ...
        """
        return (-1)*label*np.log(prediction) - (1-label)*np.log(1-prediction)
    ...
    def backward(self, inputs, diff):
        ...
        """
        self.weights[0] = self.weights[0] + 0.01*diff*inputs[0]
        self.weights[1] = self.weights[1] + 0.01*diff*inputs[1]
        self.weights[2] = self.weights[2] + 0.01*diff
        #print(self.weights)
